...
void    child_main(int fdp[], int fdc[], int seed)
{
    ...
    int max = gmn_get_max();
    if (write(fdc[PIPEFD_WRITE], &max, sizeof(int)) == -1) {
        die("write()");
    }
    ...
    while (check != 0) {
        ...
        if (write(fdc[PIPEFD_WRITE], &check, sizeof(int)) == -1) {
            die("write()");
        }
    }
    //  send the final message back (as a string) 
    char* msg = gmn_get_message(&gmn);
    if (write(fdc[PIPEFD_WRITE], msg, MSG_BUF_SIZE) == -1) {
        die("write()");
    }
...
int main(int argc, char *argv[])
{
    ...
    do { 
        ...
        if (write(fdp[PIPEFD_WRITE], &guess, sizeof(int)) == -1) {
            die("write()");
        }
    ...
    read(fdc[PIPEFD_READ], &msg, MSG_BUF_SIZE);
