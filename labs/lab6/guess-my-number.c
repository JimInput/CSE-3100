...
void    child_main(int fdp[], int fdc[], int seed)
{
    ...
    int max = gmn_get_max();
    if (write(fdc[1], &max, sizeof(int)) == -1) 
        die("write() 1");
    ...
    int check = -1;
    while(check != 0) {
        if (read(fdp[0], &guess, sizeof(int)) == -1) 
            die("read() 1");

        check = gmn_check(&gmn, guess);

        if (write(fdc[1], &check, sizeof(int)) == -1) {
            die("write() 2");
        }
    ...
    if (write(fdc[1], &gmn.message, MSG_BUF_SIZE))
...
int main(int argc, char *argv[])
{
    ...
    if (read(fdc[0], &max, sizeof(int)) == -1)
        die("read() 2");
    ...
    do { 
        ...
        if (write(fdp[1], &guess, sizeof(int)) == -1)
            die("write() 2");
        //     wait for the result from the child
        if (read(fdc[0], &result, sizeof(int)) == -1) 
            die("read() 3");
    ...
    if (read(fdc[0], &msg, MSG_BUF_SIZE) == -1) {
        die("read() 4");
    }
    printf("%s", msg);
    //      close all pipe file descriptors
