...
void * thread_c(void * arg_in)
{
    ...
    do {
        ...
        if (arg->result == 0)
            strcpy(arg->message, gmn.message);

        pthread_cond_signal(&arg->cond_result);
        arg->status = S_RESULT;
        // printf("c: sent result\n");
    } while (arg->result != 0);
...
void * thread_p(void *arg_in)
{
    ...
    // printf("p: in producer, s=%d\?n", arg->status);
    pthread_mutex_lock(&arg->mutex);
    while (arg->status != S_MAX) {
        // printf("p: waiting for max\n");
        pthread_cond_wait(&arg->cond_result, &arg->mutex);
    }
    // printf("p: done waiting for max\n");
    max = arg->max;
...
int main(int argc, char *argv[])
{
    ...
    pthread_cond_init(&cond_guess, NULL);
    pthread_cond_init(&cond_result, NULL);

    arg.status = S_INIT;
    arg.mutex = mutex;
    arg.cond_guess = cond_guess;
    arg.cond_result = cond_result;

    pthread_create(&producer, NULL, thread_p, &arg);
    pthread_create(&consumer, NULL, thread_c, &arg);
