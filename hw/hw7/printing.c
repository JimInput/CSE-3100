...
{
    printer_t* info = (printer_t*) arg;
    // TODO
    int done = 0;
    while (!done) {
        pthread_mutex_lock(&info->jq->mutex);
        int num_jobs = q_num_jobs(info->jq);
        if (num_jobs == 0) {
            pthread_mutex_unlock(&info->jq->mutex);
            done = 1;
            return arg;
        }
        int length = q_fetch_job(info->jq, info->id);
        pthread_mutex_unlock(&info->jq->mutex);
        print_job(length);
        info->njobs++;
    }
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < num_printers; i++) {
        ...
        pthread_create(&threads[i], NULL, printer_main, &printers[i]);
    }
