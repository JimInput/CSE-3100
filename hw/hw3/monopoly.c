...
int one_round(int m, int n, TPlayer p[], TProperty prop[])
{
	for(int i = 0; i < m; i++)
	{
		...
		} else if (prop[p[i].loc].owner_id != i) { // someone else owns the property
			int successful = transaction(&p[i], &p[prop[p[i].loc].owner_id], prop[p[i].loc].rent);
			if (successful == 0) {
				return 0;
			}
		}
